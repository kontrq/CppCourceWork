...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label3->Text = L"Выберите класс";
			// 
			// label4
			// 
			...
			this->label5->Location = System::Drawing::Point(298, 9);
			this->label5->Name = L"label5";
			this->label5->Size = System::Drawing::Size(106, 13);
			this->label5->TabIndex = 23;
			this->label5->Text = L"Учебные предметы";
			// 
			// listBox1
			// 
...
private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	{
		MySqlDataReader^ dataReader = MySqlWork::ExecuteQuery("SELECT * FROM users INNER JOIN ucheniki ON users.login=ucheniki.login WHERE users.login='" + textBox1->Text + "'");
		MySqlDataReader^ dataReader1 = ExecuteQuery("SELECT lessons from class WHERE class = '" + comboBox1->Text + "'");
		if (dataReader == nullptr) {
			MySqlWork::ExecuteQuery("INSERT INTO users(login,password,role) VALUES('" + textBox1->Text + "','" + MySqlWork::GetHash(textBox2->Text) + "','user')");
			MySqlWork::ExecuteQuery("INSERT INTO ucheniki(login,name,class) VALUES('" + textBox1->Text + "','" + textBox4->Text + "', '" + comboBox1->Text + "')");
